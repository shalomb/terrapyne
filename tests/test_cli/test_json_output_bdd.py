...
def projects_exist():
    ...
    api.list.return_value = (
        iter([Project.model_construct(id="prj-1", name="proj", created_at=None, resource_count=5)]),
        1,
    )
...
def req_run_list(mock_client):
    with (
        ...
        return runner.invoke(
            app, ["run", "list", "-w", "my-app-dev", "-o", "test-org", "--format", "json"]
        )
...
def req_prj_detail(mock_client):
    with (
        ...
        return runner.invoke(
            app,
            ["project", "show", "Core Infrastructure", "-o", "test-org", "--format", "json"],
