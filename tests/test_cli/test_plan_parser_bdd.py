...
def test_parse_indexed_address():
    pass
...
def test_parse_computed_markers():
    pass


@scenario(
    "../features/plan_parser_resources.feature", "Analysis of infrastructure import operations"
)
def test_parse_import_action():
    pass
...
def test_extract_data_source_error():
    pass
...
@given(
    parsers.parse('the plan indicates a new resource "{address}" will be created'),
    target_fixture="plan_text",
)
@given(
    parsers.parse('the plan indicates a change for resource "{address}"'),
    target_fixture="plan_text",
)
...
@given(
    parsers.parse('the plan indicates resource "{address}" will be destroyed'),
    target_fixture="plan_text",
)
def plan_indicates_destruction(address):
    return make_plan(
        f'  # {address} will be destroyed\n  - resource "type" "name" {{}}',
        "Plan: 0 to add, 0 to change, 1 to destroy.",
    )


@given(
    parsers.parse('the plan indicates resource "{address}" will be modified'),
    target_fixture="plan_text",
)
def plan_indicates_modification(address):
    return make_plan(
        f'  # {address} will be updated in-place\n  ~ resource "type" "name" {{}}',
        "Plan: 0 to add, 1 to change, 0 to destroy.",
    )


@given(
    parsers.parse('the plan indicates resource "{address}" must be replaced'),
    target_fixture="plan_text",
)
def plan_indicates_replacement(address):
    return make_plan(
        f'  # {address} must be replaced\n-/+ resource "type" "name" {{}}',
...
def plan_contains_ansi():
    return make_plan(
        '\x1b[1m  # aws_instance.web\x1b[0m will be created\n  + resource "aws_instance" "web" {}'
    )
...
def resource_with_attributes_table(datatable):
    ...
    for row in datatable:
        attrs += f"      + {row[0]} = {row[1]}\n"
    return make_plan(
        f'  # aws_instance.web will be created\n  + resource "aws_instance" "web" {{\n{attrs}    }}'
    )
...
@given(
    parsers.parse('the plan indicates an attribute change "{change_line}"'),
    target_fixture="plan_text",
)
def resource_with_attribute_change_declarative(change_line):
    return make_plan(
        f'  # aws_instance.web will be updated in-place\n  ~ resource "aws_instance" "web" {{\n      {change_line}\n    }}'
    )
...
@given(
    parsers.parse('the plan indicates an array attribute "{attr_line}"'), target_fixture="plan_text"
)
...
@given(
    parsers.parse('the plan indicates a nested block "{block_line}"'), target_fixture="plan_text"
)
...
@given(
    parsers.parse('the plan indicates a value will be "{v1}" or "{v2}"'), target_fixture="plan_text"
)
...
@given(
    parsers.parse('the plan indicates resource "{address}" will be imported'),
    target_fixture="plan_text",
)
def resource_with_import_declarative(address):
    return make_plan(
        f' <= {address} will be imported\n    resource "type" "name" {{\n      + ami = "ami-12345"\n    }}',
...
@given(
    parsers.parse('the plan failed due to an invalid variable value "{value}"'),
    target_fixture="plan_text",
)
...
@given(
    parsers.parse('I have a terraform plan output file "{filename}"'), target_fixture="plan_file"
)
