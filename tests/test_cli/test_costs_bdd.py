...
def mock_api_client():
    ...
    def mock_resolve_project(client, org=None, project_name=None):
        ...
        prj = Project(
            id="prj-default", name=project_name or "default", organization={"name": "test-org"}
        )
        return "test-org", prj
...
@given(
    parsers.parse(
        'the latest run for "{workspace_name}" has a cost estimate of ${monthly} monthly with a ${delta} delta'
    )
)
def latest_run_has_cost_estimate(
    mock_api_client: MagicMock, workspace_name: str, monthly: str, delta: str
) -> None:
...
def latest_run_has_cost_estimate_decrease(
    mock_api_client: MagicMock, workspace_name: str, monthly: str, delta: str
) -> None:
    mock_api_client.runs.get_latest_cost_estimate.return_value = {
        "proposed-monthly-cost": f"{monthly}.00",
...
@given(
    parsers.parse(
        'the project "{project_name}" contains workspaces with cost estimates totaling ${total_monthly} monthly'
    )
)
def project_workspaces_have_cost_estimates(
    mock_api_client: MagicMock, project_name: str, total_monthly: str
) -> None:
    ...
    def get_costs(workspace_id, **kwargs):
        if workspace_id == "ws-1":
            return {
                "proposed-monthly-cost": str(int(total_monthly) // 2) + ".00",
            ...
            return {
                "proposed-monthly-cost": str(int(total_monthly) - int(total_monthly) // 2) + ".00",
                "delta-monthly-cost": "0.0",
                "matched-resources-count": 0,
                "unmatched-resources-count": 0,
                "resources": {"matched": [], "unmatched": []},
...
@given(
    parsers.parse('the project "{project_name}" contains workspaces with invalid cost estimates')
)
def project_workspaces_invalid_cost_estimates(
    mock_api_client: MagicMock, project_name: str
) -> None:
    mock_api_client.workspaces.list.return_value = ([Workspace(id="ws-1", name="ws-1")], 1)
    mock_api_client.runs.get_latest_cost_estimate.return_value = {
...
@then(
    parsers.parse(
        'the output should show the total project estimated monthly cost of "{expected_cost}"'
    )
)
